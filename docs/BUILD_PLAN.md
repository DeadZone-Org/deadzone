...
- **2026-06-04 — Phase 3 COMPLETE (real ERC-8004).** Deployed the 3 registries (`DeadzoneRegistry.sol`: IdentityRegistry ERC-721, ValidationRegistry pre-commit→reveal, ReputationRegistry) + redeployed token/settlement as one clean full stack. **The agent now does REAL on-chain ERC-8004 writes** — mint identity NFT, pre-commit decision hash before settling, attest outcome + reputation after. Proven live: courier **agent #1** minted identity, pre-committed (`0x6c997139…`), settled 100 dUSD (`0x032a8135…`), attested (`0x2ccbb38a…`), on-chain reputation = 1 delivery / score 100. 8 Foundry + 5 agent tests pass, typecheck clean.
  - **Canonical full-stack addresses (Mantle Sepolia 5003):** DeadzoneToken `0xEF1ec1FeA446E6a7869221F00c9DC76306edca54` · DeadzoneSettlement `0xF817E6947CC94559e3e6AfF9f06fe938C3A0c652` · IdentityRegistry `0x6dd59064BC298BA85AA11a2953DA2BaA92B46382` · ValidationRegistry `0x936D1Aa670590767070Cb9Bde0264aA4d1543275` · ReputationRegistry `0x94ddC4368F8ac592Ad41067F11E43D43CDd65d94`. (Earlier deploys superseded.) Deployer topped up to ~3.9 MNT.
- **2026-06-04 — Phase 4 COMPLETE (web dApp + gateway).** `gateway/` Express API wraps the agent (`/api/status`, `/api/pay`); `web/` Vite+React tactical-HUD dApp (mesh radar, live agent-reasoning console, dynamic ERC-8004 courier card, sim fallback). Verified browser→gateway→GLM→real Mantle settlement; build clean.
- **2026-06-04 — Phase 5 COMPLETE (code) — Android offline app + gateway relay/faucet.** Added `/api/relay` (settle a phone-signed auth off the mesh) + `/api/faucet` (mint demo dUSD); **verified live**: fresh wallet → faucet → sign offline → relay → GLM settles (42 dUSD moved). `mobile/` Expo Android app reuses NONET's BLE stack (`bleUtils.ts` verbatim), `useMesh` hook (fragment/broadcast/reassemble + gateway-forward), offline EIP-3009 signing (device key in AsyncStorage), Deadzone theme, Send+Wallet screens, BLE permissions plugin. **Typechecks clean** (can't run Android build/BLE here — needs a machine w/ Android SDK + 2 phones; see `mobile/README.md`).
- **Next (requires user/hardware):** (1) build the Android app on a machine w/ Android SDK + run the 2-phone offline demo → record the ≥2-min video; (2) verify the 5 contracts on Mantlescan (needs MANTLESCAN_API_KEY) for the 20-Project Deployment award; (3) deploy `web/` to a public URL (Vercel/Netlify) + host the gateway. All code phases (0–5) are done.
